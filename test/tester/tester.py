...
if __name__ == "__main__":
    ...
    basic_tests_total = 0
    test_battery = [("Small GET test", "2s/10 connections"),
                    ("Medium GET test", "5s/200 connections"),
                    ("Big GET test", "10s/1000 connections"),
                    ("Medium GET Internet test", "5s/500 connections simulating internet"),
                    ("Big GET Internet test", "10s/1000 connections simulating internet")]
    for test in test_battery:
        ...
        match test[0]:
            ...
            case "Medium GET Internet test":
                result = tests.medium_internet_get_test()
            case "Big GET Internet test":
                result = tests.big_internet_get_test()
            case _:
                pass
        if (webserv.returncode):
    ...
    print("Tests passed: ")
    print("\t Basic:" + colors[("Red", "Green")[basic_tests_passed == basic_tests_total]],
          f"{basic_tests_passed}/{basic_tests_total}",
           f"({basic_tests_passed / basic_tests_total * 100}%)" + colors["Reset"])
